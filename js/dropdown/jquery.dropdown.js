...
(function ($) {
  ...
  var settings = {
    ...
    limitCount: Infinity,
    noDataMsg:'数据列表为空',
    input: '<input type="text" maxLength="20" placeholder="搜索关键词或ID" class="search">',
    data: [],
    searchable: true,
    searchNoData: '<li style="color:#ddd">查无数据，换个词儿试试 /(ㄒoㄒ)/~~</li>',
    choice: function choice() {},
    showName:function(name){return name}
  };
  ...
  function createTemplate() {
    ...
    var btnstr         = isSingle?'':'<button type="button" class="dropdown-search-btn add">全选</button>';
    var templateSearch = searchable ? '<span class="dropdown-search">' + this.config.input + btnstr +'</span>': '';
  ...
  function maxItemAlert() {
    ...
    if ($alert.length === 0) {
      if (_config.limitCount == Infinity)
        $alert = $('<div class="dropdown-maxItem-alert">' + _config.noDataMsg + '</div>');
      else
  ...
  function objectToSelect(data) {
    ...
    $.each(data, function (index, val) {
      ...
      if (isSelected) {
        var itemName = _config.showName(val.name);
        name.push('<span class="dropdown-selected">' + itemName + '<i class="del" data-id="' + val.id + '"></i></span>');
        selectAmount++;
      }
  ...
  function selectToObject(el) {
    ...
    $.each($select.children(), function (key, el) {
      ...
      if (tmp.id) {
        ...
        _dropdown.placeholder = tmp.name;
      }
    });
  ...
  var action = {
    ...
    search: throttle(function (event) {
      ...
      var intputValue = $.trim($input.val()).toLowerCase();
      ...
      $.each(data, function (key, value) {
        if (value.name.toLowerCase().indexOf(intputValue) > -1) {
          result.push(value);
        }
      });

      $el.find('ul').html(selectToDiv.call(_dropdown,objectToSelect.call(_dropdown, result)[0]) || _config.searchNoData);
    }, 300),
    ...
    multiChoose: function multiChoose(event) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
          var itemName =  _config.showName(item.name);
          selectedName.push(itemName);
          _dropdown.name.push('<span class="dropdown-selected">' + itemName + '<i class="del" data-id="' + item.id + '"></i></span>');
        }
      });
    ...
    selectAll: function selectAll(event) {
        ...
        _dropdown.name = [];
        $el.find('ul li.dropdown-option').each(function(key, item) {
            var itemName  = _config.showName($(this).text());
            var itemValue = $(this).attr('data-value');
            _dropdown.name.push('<span class="dropdown-selected">' + itemName + '<i class="del" data-id="' + item.id + '"></i></span>');
            $select.find('option[value="' + itemValue + '"]').prop('selected', true);
            $.each(_dropdown.config.data, function (key1, item1) {
              if ('' + item1.id == '' + itemValue) {
                item1.selected = true;
                return;
              }
          });
        });
        $el.find('ul li.dropdown-option').addClass('dropdown-chose');
        _dropdown.name.push('<span class="placeholder">' + _dropdown.placeholder + '</span>');
        _dropdown.$choseList.html(_dropdown.name.join(''));
        return false;
    },
    clearAll: function clearAll(event) {
      ...
      this.$el.find('ul li.dropdown-option').removeClass('dropdown-chose');
      if (this.isSingleSelect) {
        this.$select.val('');
      } else {
        this.$select.find('option').prop('selected', false);//清除所有数据选择状态 2017.10.10
      }
  ...
  Dropdown.prototype = {
    init: function init() {
      ...
      if (_config.data.length === 0) {
        _config.data = selectToObject.call(_this, _this.$select);
      }
      ...
      if (_this.isSingleSelect) {
        var sval = _this.$select.find("option[selected]").val();
        _this.$select.prepend("<option>"+_this.placeholder+"</option>");
        if (sval)
            _this.$select.val(sval);
        else
            _this.$select.val('');
      }
    },
    ...
    bindEvent: function bindEvent() {
      ...
      if (_this.isLabelMode) {
        ...
        if (!_this.placeholder) //单选无空项，禁止清除
            $('div.dropdown-single a.dropdown-clear-all').remove();

      }

      // 搜索
      $el.on(EVENT_SPACE.keyup, 'input', $.proxy(action.search, _this));
      $el.on(EVENT_SPACE.click, 'button.add', $.proxy(action.selectAll, _this));
      var pwidth = $el.width();
      $el.find('input.search').each(function(){
        if (pwidth && pwidth>70) {
            $(this).width(pwidth-70);
        }
      });
      //$el.on(EVENT_SPACE.click, 'button.del', $.proxy(action.clearAll, _this));
      // 按下enter键设置token
    ...
    clearAll:function() {
        action.clearAll.call(this,event);
    }
  };
