...
var styleInfo = ['现代简约', '欧式风格', '北欧风格', '中式风格', '简欧风格', '新中式风格', '后现代工业风', '奢华三居'];
var stylePrice = [69800, 89800, 69800, 89800, 69800, 89800, 79800, 79800];
Page({
  ...
  styleChange: function (e) {
    var that = this;
    var styelArray = that.data.styelArray;
    var styleType = that.data.styleType;
    var stylePrice = that.data.stylePrice;
    this.setData({
      styleNum: e.detail.value,
      styleType: styelArray[e.detail.value],
      stylePrice: stylePrice[e.detail.value]
    })
  },
  //展示选择装修户型列表
  homeshow: function () {
  ...
  sendInfo: function (e) {
    ...
    postInfo['style'] = that.data.styleType;
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.status == 200) {
          if (area > 80) {
            if (that.data.toiletNum > 1) {
              totalPrice = stylePrice + ((area - 80) * 1000) + ((that.data.toiletNum - 1) * 10000);
            } else {
              totalPrice = stylePrice + ((area - 80) * 1000);
            }
          } else {
            if (that.data.toiletNum > 1) {
              totalPrice = stylePrice - ((80 - area) * 1000) + ((that.data.toiletNum - 1) * 10000);
            } else {
              totalPrice = stylePrice - ((80 - area) * 1000);
            }
          }
          wx.showModal({
            title: 'OKE家预估报价',
            content: totalPrice + '元',
            showCancel: false,
            success: function () {
              wx.showToast({
                title: '提交成功',
